# Local Agent Stack

This folder stores the safe, GitHub-trackable setup notes for the local Luna,
Star, Solar, Hermes, Honcho, Paperclip, and Ouroboros stack.

No bot tokens, API keys, OAuth tokens, or local auth-state files belong here.

## Files

- `luna-star-solar-setup.md`: operational setup and routing policy.
- `openclaw-agent-stack.example.json`: sanitized OpenClaw config excerpt.
- `tokenless-github-policy.md`: what is safe to commit and what must stay local.
